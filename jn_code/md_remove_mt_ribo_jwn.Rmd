...
## percent.mt and percent.ribo

```{r, echo=TRUE, warning=FALSE, error=FALSE, message=FALSE}

SO[["percent.ribo"]] <- PercentageFeatureSet(SO, pattern = "^Rp")

head(SO@meta.data)

VlnPlot(SO, features = c("percent.ribo", "percent.mt"), group.by = "orig.ident")

VlnPlot(SO, features = c("percent.ribo"))

FeaturePlot(SO, "percent.ribo")
...
SO2 <- SO[!grepl("^mt-", rownames(SO)), ]
SO2 <- SO2[!grepl("^Rp", rownames(SO2)), ]
...
DimPlot(SO3, group.by = "treatment")

...
pos.markers2 <-  markers2 %>% dplyr::filter(avg_log2FC > 0.85) %>%  arrange(desc(abs(avg_log2FC)))
#head(pos.markers, n = 50)
...
pos.markers3 <-  markers3 %>% dplyr::filter(avg_log2FC > 0) %>%  arrange(desc(abs(avg_log2FC)))
#head(pos.markers, n = 50)
...
# Save the workbook
saveWorkbook(wb, here("jk_code", "Combined_Pathways_MD_DEGs.xlsx"), overwrite = TRUE)
